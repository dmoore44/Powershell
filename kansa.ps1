...
This is a fork of the original kansa.ps1. "Improvements" have been made 
by adding two additional output formats: GL and SPLUNK.  Both allow the
user to route data output to a GrayLog or Splunk instance respectively,
while also writing the output to JSON files as specified by the default 
JSON output format.
...
The GL output format definition begins on line 722 and the user needs to 
configure the $glserver and $glport variables.  The facility parameter
in the message body should also be configured by the user... Unless the
user is fine with the pre-defined facility.

The SPLUNK output format definition begins on line 742 and the user needs
to configure the $token, $server, and $port variables.
